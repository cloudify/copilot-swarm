...
import { humanizeTime } from "./textUtils.js";
...
// Helper function to humanize age
const humanizeAge = (ts: Date): string => {
...
// Helper function to humanize remaining time
const humanizeRemaining = (delta: number): string => {
  const seconds = Math.floor(delta / 1000);
  if (seconds <= 0) {
    return "0m";
  }
