...
interface ToolParser {
  name: string;
  patterns: RegExp[];
  parse: (line: string, match: RegExpMatchArray, allLines: string[], lineIndex: number) => ParsedError | null;
}
...
const TOOL_PARSERS: ToolParser[] = [
    ...
    parse: (line, match, allLines, lineIndex) => {
      ...
      if (match.length >= 4) {
        ...
        return {
          ...
          severity: (match[3] === "warn" ? "warning" : match[3] as "error" | "warning" | "info") || "error",
  ...
  },
  {
    ...
    parse: (line, match, allLines, lineIndex) => {
      ...
      if (line.includes("expect(received)") || line.includes("Expected:") || line.includes("Received:")) {
...
function extractJestErrorContext(allLines: string[], lineIndex: number): { message: string; lines: string[] } {
  ...
  for (let i = Math.max(0, lineIndex - 15); i < lineIndex; i++) {
    ...
    if (line && line.includes("expect(") && (line.includes(").toBe(") || line.includes(").toContain(") || line.includes(").toEqual("))) {
      message = line.trim();
    }
    if (line && (line.includes("Expected:") || line.includes("Received:") || line.includes("expect(received)"))) {
...
function extractErrorContext(allLines: string[], lineIndex: number, contextSize = 2): string[] {
  const start = Math.max(0, lineIndex - contextSize);
  const end = Math.min(allLines.length, lineIndex + contextSize + 1);
  return allLines.slice(start, end).map(line => line.trim()).filter(line => line.length > 0);
}

function parseLogLine(line: string, allLines: string[], lineIndex: number): ParsedError | null {
  ...
  if (
    ...
    let tool = "unknown";
    if (lowerLine.includes("npm") || lowerLine.includes("yarn")) {tool = "npm";}
    else if (lowerLine.includes("webpack")) {tool = "webpack";}
    else if (lowerLine.includes("jest")) {tool = "jest";}
    else if (lowerLine.includes("eslint")) {tool = "eslint";}
    else if (lowerLine.includes("tsc") || lowerLine.includes("typescript")) {tool = "tsc";}
...
export function extractErrorsFromJobLogs(logText: string, _jobName?: string): ErrorExtraction {
  ...
  return {
    ...
    getPrioritizedErrors(): ParsedError[] {
      return [...errors].sort((a, b) => {
        ...
        const toolOrder = { tsc: 0, webpack: 1, jest: 2, eslint: 3, npm: 4, unknown: 5 };
        const toolDiff = (toolOrder[a.tool as keyof typeof toolOrder] || 5) - 
    ...
    getGroupedErrors(): Record<string, ParsedError[]> {
      ...
      for (const error of errors) {
        const key = error.file || error.tool;
        if (!grouped[key]) {
          grouped[key] = [];
        }
    ...
    generateSummary(): string {
      ...
      if (prioritized.length > 0) {
        ...
        for (const error of prioritized) {
          ...
          summaryLines.push(`- ${error.message}${location}`);
        }
      }
...
function extractSummaryInfo(logText: string, _errors: ParsedError[]): string {
  ...
  for (const line of lines) {
    ...
    // Jest summary
    if (trimmed.includes('Test Suites:') && (trimmed.includes('failed') || trimmed.includes('passed'))) {
      summaryLines.push(trimmed);
    }
    if (trimmed.includes('Tests:') && (trimmed.includes('failed') || trimmed.includes('passed'))) {
      summaryLines.push(trimmed);
    }
    ...
    // Webpack summary
    if (trimmed.includes('webpack') && trimmed.includes('compiled with') && trimmed.includes('error')) {
      summaryLines.push(trimmed);
    }
...
export async function collectFailureLogsAdvanced(
  ...
  const debugLog = (message: string, data?: any) => {
    console.log(`[Error Extraction Debug] ${message}`, data ? JSON.stringify(data, null, 2) : "");
  };
          ...
          if (typeof jobLogText === "string" && jobLogText.length > 0) {
            ...
            debugLog(`Advanced error extraction for job ${job.id}`, {
              ...
              toolCounts: Object.fromEntries(
                Object.entries(extraction.errorsByTool).map(([tool, errors]) => [tool, errors.length])
              ),
            });

            if (extraction.errors.length > 0) {
              // Generate a comprehensive error report
              const prioritizedErrors = extraction.getPrioritizedErrors().slice(0, 10);
              ...
              if (prioritizedErrors.length > 0) {
                ...
                for (const [fileOrTool, errors] of Object.entries(grouped)) {
                  if (processedFiles.has(fileOrTool)) {continue;}
                  ...
                  if (relevantErrors.length > 0) {
                    ...
                    for (const error of relevantErrors) {
                      ...
                      errorLines.push(`- ${error.severity}: ${error.message}${location}${code}${rule}`);
                    }
                  }
                }
              }
